# NEXUS — AI Vehicle Maintenance Assistant (Local LLM & RAG)

본 프로젝트는 외부 유료 API나 복잡한 클라우드 인프라 의존성 없이, Docker 환경에서 로컬 LLM(Ollama - Llama 3 / Llava 멀티모달 비전 모델)과 RAG(Retrieval-Augmented Generation, LangChain, ChromaDB) 기술을 유기적으로 결합하여 구동되는 독립형 AI 차량 정비 어시스턴트 시스템입니다.

단순한 텍스트 기반 챗봇을 넘어, 볼보(Volvo XC60)를 비롯한 다양한 브랜드와 차종의 소모품(에어컨/캐빈 필터, 에어크리너, 브레이크 패드, 프론트 와이퍼 등) 상태를 진단하고 공식 매뉴얼에 기반한 정확한 정비 가이드를 제공합니다. 향후 멀티 브랜드/차종 통합 확장부터 맞춤형 화이트 라벨 솔루션까지 무한히 확장할 수 있는 아키텍처로 설계되었습니다.

---

## 데모 및 빠른 시작 안내 (Demo Ready)
본 포트폴리오 프로젝트는 원활한 시연(Demo) 및 즉시 테스트를 위해 다음의 로컬 파일들을 Git 저장소에 포함하고 있습니다:
- **`backend/app.db`**: 테스트 계정 및 사전 등록된 차량 정비 이력 데이터베이스
- **`backend/uploads/`**: 정비 이력 상세 보기에 사용되는 테스트 차량 부품 이미지 파일
- **`backend/weights/` (또는 학습 가중치)**: 다중 부품 인식을 위한 YOLOv8 커스텀 학습 모델 가중치 파일 (`front_wiper`, `air_cleaner`, `brake_pad`, `brake_disc`, `spark_plug`, `oil_filter` 등 7개 클래스 확장 완료)

> 🔑 **테스트 계정 정보**
> - **아이디**: `test`
> - **비밀번호**: `1234`
> - 제공된 계정으로 로그인하여 미리 등록된 정비 이력을 확인하거나, 직접 회원가입하여 새로운 부품 이미지를 업로드하고 진단하는 전체 프로세스를 시연하실 수 있습니다.

---

1. 프로젝트 기획 배경 및 동기 (Why We Built This)

현업의 문제점 체감: 자동차 정비 현장에서 일하면서 간단한 소모품 교체(에어컨 필터, 와이퍼 등)는 공임비를 포함하면 실제 부품 비용에 비해 적지 않은 비용이 발생합니다. 많은 운전자들이 방법을 몰라서, 혹은 번거로워서 비용을 지불하고 서비스를 이용하지만, 실상은 누구나 가이드를 통해 직접 해결할 수 있는 비교적 간단한 작업들입니다. 고물가 시대에 운전자들이 불필요한 공임비 부담을 덜고 실용적으로 차량을 관리할 수 있는 도구가 필요하다고 판단했습니다.

B2B 및 서비스 생태계 관점의 가치 창출:
- 고객 측면: 정비 진입장벽을 낮춰 경제적 부담을 줄이고, 다양한 차종의 신뢰성 높은 DIY 가이드와 커뮤니티 서비스를 누립니다.
- 브랜드/정비소 측면: 단순 소모품 교체로 인해 공식 서비스 센터의 예약 슬롯이 낭비되는 병목 현상을 완화하고, 전문적인 진단과 수리가 필요한 중증 결함 차량에 서비스 리소스를 집중하여 예약 주기를 단축할 수 있습니다.
- 비즈니스 확장성: 다양한 브랜드와 차종을 아우르는 범용 플랫폼 기반 위에, 완성차 브랜드별 니즈에 맞춘 맞춤형 화이트 라벨 솔루션으로 확장할 수 있는 기회를 제공합니다.
- 프라이버시와 독립성 확보: 상용 클라우드 AI를 쓸 때 발생하는 차량 정보 및 개인정보 유출 우려를 원천 차단하기 위해, 로컬 환경에서 안전하게 구동되는 독립형 정비 어시스턴트를 구축하고자 본 프로젝트를 기획하게 되었습니다.

2. 프로젝트 주요 특징 및 기술 스택
- Role: Full-Stack Architecture Design & Local AI Pipeline Orchestration
- Language & Framework: Python, FastAPI, React, Vite, Tailwind CSS, LangChain, ChromaDB
- AI Core: Ollama (Llama 3 / Llava 멀티모달 비전 모델), YOLOv8 (프론트 와이퍼 등 커스텀 부품 감지 및 멀티 부품 가중치 최적화)
- Database & Security: SQLite (app.db), SQLAlchemy, bcrypt 인증 모듈 연동
- Containerization: Docker, Docker Compose

3. 시스템 아키텍처 및 모듈화 구조 (Modular Architecture & Data Architecture)

본 프로젝트는 차량 부품 데이터의 파편화를 방지하고 향후 다양한 브랜드 및 소모품 지속 확장에 유연하게 대응하기 위해, 데이터 소스와 서비스 로직을 완전히 분리한 모듈형 아키텍처로 설계되었습니다.

소스코드 모듈화 구조 (backend/ 및 frontend/):
- main.py: FastAPI 기반의 모던 웹 서비스 백엔드 API 서버 엔트리포인트 (인증, 진단, 챗봇, 사용자 설정 관리 통합)
- auth.py: SQLite 데이터베이스 및 bcrypt를 활용한 안전한 사용자 회원가입, 로그인 및 세션 보안 관리 모듈
- database.py 및 models.py: SQLAlchemy 기반 데이터베이스 연결 세션 관리 및 유저 정보, 사용자별 정비 진단 이력 등 DB 스키마 모델 정의
- vision_module.py: YOLOv8 기반 커스텀 부품 인식 및 시각적 진단 처리 (front_wiper 등 추가 학습 적용)
- rag_module.py: ChromaDB 벡터 스토어 및 Ollama 기반 로컬 LLM 텍스트 RAG 파이프라인
- utils.py: 스마트 파일명 우선 검색(Exact Match) 로직 및 데이터 유틸리티 처리 (부품 인식과 가이드 매칭 정확도 극대화)
- frontend/: React와 Vite, Tailwind CSS, Lucide Icons를 기반으로 구축된 모던 반응형 사용자 인터페이스 (대시보드, 챗봇, 정비 진단, 설정 및 모달 관리)

지식베이스 분리 및 파이프라인 (manual/ 및 yolo_training_data/):
- 정비 매뉴얼의 원본 Markdown(.md) 문서를 파싱하여 구조화된 JSON(.json) 데이터로 변환하는 스크립트(convert_to_json.py)와, 이를 ChromaDB 벡터 스토어에 임베딩 및 적재하는 파이프라인(ingest.py)을 분리 구축했습니다.
- 구조화된 차량별 상세 JSON 스키마 도입 (manual/volvo/xc60/): 에어컨/캐빈 필터, 에어크리너, 프론트 와이퍼 등 소모품별 정비 가이드를 단계별(steps)로 체계화한 상세 JSON 스키마를 적용했습니다. 각 단계별로 설명(description), 경고(warning), 핵심 팁(key_point)뿐만 아니라 단일 또는 멀티 이미지 경로(image)를 매핑하여, 사용자가 직관적인 시각적 가이드를 확인할 수 있도록 데이터 구조를 고도화했습니다.
- 브랜드/차종별 전용 폴더(manual/브랜드명/차종명/)에 격리 관리함으로써 새로운 차종이나 소모품이 추가되더라도 메인 비즈니스 로직 수정 없이 데이터 인프라를 무한히 확장할 수 있습니다.

도커라이징 및 환경 최적화:
- opencv-python-headless 적용을 통해 리눅스 기반 컨테이너 환경에서 발생할 수 있는 GUI 의존성(libGL.so.1) 오류를 원천 차단하고 안정적인 비전 파이프라인을 구축했습니다.
- 정비 가이드 상세 이미지 에셋(manual_images/)과 런타임 동적 데이터(진단 이미지 영구 저장을 위한 uploads/, vector_db/, runs/)를 프로젝트 루트 레벨에서 안전하게 격리·관리하도록 구성했습니다.
- requirements.txt에 FastAPI, Uvicorn, SQLAlchemy, PyJWT, bcrypt, python-multipart 등 모던 웹 서비스 구동을 위한 필수 패키지 의존성을 최신화하여 반영했습니다.

4. 핵심 구현 기능 (Core Features)

로컬 멀티모달 비전 정비 어시스턴트 및 대화형 RAG
- 현재 상태: RAG 기반 정비 매뉴얼 텍스트 질의응답 및 로컬 환경 연동 구현 완료.
- 고도화 방향 및 구현 완료: 사용자가 부품 사진을 업로드하면 커스텀 학습된 YOLOv8 모델(best.pt)이 프론트 와이퍼(front_wiper), 에어크리너 등 부품 종류를 정확하게 시각적으로 식별하고, 최적화된 파일 매칭 로직을 통해 이에 정확히 매칭되는 차량별 상세 JSON 가이드를 불러와 소요 시간, 필요 공구, 각 단계별(Step-by-step) 상세 이미지와 주의사항을 UI에 유기적으로 렌더링하는 파이프라인을 구축했습니다. 또한 확대 모달(ImageModal)을 통해 업로드 이미지 및 가이드 사진의 100%~500% 줌 및 드래그 탐색을 지원합니다.

서버 기반 이미지 영구 저장 및 사용자별 진단 이력 영속성 (Permanent Storage & History Persistence)
- 서버 기반 이미지 영구 저장 및 최적화: 업로드 직후 화면에 임시로 표시되던 브라우저의 휘발성 blob: 주소 방식에서 벗어나, 백엔드 서버(uploads/ 디렉터리)에 진단 이미지를 물리적으로 안전하게 영구 저장하고 처리하는 로직을 구축했습니다. 프론트엔드 모달 및 채팅창 UI에서 이미지 경로 유실이나 404 에러 없이 상대·절대 경로가 안정적으로 렌더링되도록 개선했습니다.
- 정비 이력 상세 조회 및 관리: 사용자가 수행했던 정비 진단 및 챗봇 내역을 [정비 이력 관리] 탭에서 한눈에 확인하고, 기록 클릭 시 당시의 상세 정비 가이드 및 대화 내용을 곧바로 복원(불러오기)할 수 있습니다.
- 개별 이력 삭제 기능: 불필요하거나 지나간 정비 기록을 사용자가 직접 관리할 수 있도록 개별 삭제 버튼을 추가했습니다.
- 사용자별 맞춤 진단 이력 영속성 강화: 진단 결과를 데이터베이스에 기록할 때 현재 로그인된 사용자의 고유 아이디(username)를 명확히 매칭하도록 연동하여, 사용자가 로그아웃 후 재로그인하거나 페이지를 새로고침하더라도 데이터베이스(app.db)를 통해 자신의 정비 진단 이력이 유실되지 않고 완벽하게 유지·복원되도록 구현했습니다.

무중단 독립형 Docker 환경 구성
- 외부 API 호출 실패나 네트워크 지연 리스크를 원천 차단하기 위해, Ollama 엔진과 웹 인터페이스를 도커 컨테이너로 묶어 어떤 로컬 PC 환경에서도 단일 명령어로 즉시 구동되도록 패키징했습니다.

5. 현실적 리스크 분석 및 방어 전략 (Risk Management)

제조사 보증 및 책임 소재 공방 리스크
- 분석: 사용자가 DIY 작업 중 부품을 오장착하여 차량 결함이 발생할 경우 제조사 보증 거부 및 책임 문제가 대두될 수 있습니다.
- 대응 전략: 정비 범위를 안전에 지장이 없는 기본 소모품 영역으로 엄격히 한정하고, UI 및 가이드 상단에 면책 조항(Disclaimer)을 배치하여 법적 리스크를 선제적으로 방어합니다.

AI 환각(Hallucination)으로 인한 오정비 방지
- 분석: LLM의 특성상 부정확한 정비 방법을 안내할 경우 차량 손상으로 직결될 수 있습니다.
- 대응 전략: 범용 답변 생성을 차단하고, 엄격하게 검증된 공식 정비 매뉴얼 텍스트 데이터베이스(ChromaDB RAG) 기반으로만 답변을 유도하여 정보의 정확도를 극대화했습니다.

기업 내부 정보 유출 및 권한 분리 리스크 (B2B 보안)
- 분석: 일반 고객용 인터페이스와 사내 직원/정비사 전용 데이터베이스가 분리되지 않을 경우, 대외비 성격의 내부 정비 지침이나 사내 데이터가 유출될 위험이 있습니다.
- 대응 전략: 사용자 권한(RBAC) 모델을 도입하여 고객용 뷰와 사내 임직원/교육용 뷰를 철저히 분리하고, 브랜드별 전용 지식베이스는 외부 접근을 원천 차단하는 데이터 격리 아키텍처를 설계에 반영했습니다.

6. 향후 확장 계획 및 비즈니스 모델 (Scalability)

- 멀티 브랜드 및 다차종 데이터 확장 자동화 파이프라인: manual/ 하위에 브랜드별·차종별 디렉터리 체계를 구축하고, 초기 기동 시 마크다운 문서를 자동으로 파싱하여 벡터 DB에 동적 임베딩하는 자동 인덱싱 스크립트 파이프라인을 도입합니다.
- 완성차 브랜드별 맞춤형 화이트 라벨(White-Label) 및 독립 생태계 구축: 특정 완성차 브랜드와의 전략적 협업을 통해 해당 브랜드 오너들만을 위한 전용 애플리케이션으로 패키징하여 공급합니다.
- 차대번호(VIN) 기반 인앱 실시간 리콜 조회 시스템: 프로그램 내에서 차량번호 및 차대번호(VIN)를 직접 입력해 리콜 대상 여부와 공식 조치 가이드를 실시간으로 확인하는 기능을 구현합니다.
- 간헐적 소음 진단 및 유저 참여형 커뮤니티 생태계: 운전자들이 직접 소음 증상을 대조하고 자가 진단 가이드를 얻을 수 있는 브랜드별/증상별 정비 사례 공유 커뮤니티 공간을 연동합니다.
- 애프터마켓 제휴 광고 및 공식 센터 원스톱 예약 시스템: 정비 주기에 구애받지 않고, 원할 때 언제든 틴팅, 블랙박스 등 차량용품 및 애프터마켓 시공 업체를 찾아보고 연계할 수 있는 타겟팅 마케팅 모델을 구축합니다. 자가 정비 범위를 넘어선 중증 정비나 점검이 필요할 때, 애플리케이션 내에서 곧바로 공식 서비스 센터 및 연계 사설 정비소를 예약할 수 있는 편의 기능을 제공합니다.

7. 프로젝트 디렉터리 구조 (C:\DIY)

```text
diy/ (최상위 루트)
├── .git/                           # Git 버전 관리 디렉토리
├── backend/                        # FastAPI 백엔드 소스코드 및 모듈
│   ├── weights/                    # 커스텀 학습 모델 가중치 폴더
│   ├── app.db                      # SQLite 데이터베이스 파일
│   ├── auth.py                     # 사용자 인증 및 보안 관련 모듈
│   ├── convert_to_json.py          # 매뉴얼 데이터 변환 스크립트
│   ├── database.py                 # 데이터베이스 연결 및 세션 설정
│   ├── ingest.py                   # 벡터 DB(ChromaDB) 문서 적재 스크립트
│   ├── main.py                     # FastAPI 메인 서버 및 API 엔드포인트
│   ├── models.py                   # DB 스키마 및 데이터 모델 정의
│   ├── rag_module.py               # RAG 검색 및 벡터 DB 연동 모듈
│   ├── requirements.txt            # 파이썬 패키지 의존성 목록
│   ├── train_yolo.py               # YOLO 모델 학습 실행 스크립트
│   ├── utils.py                    # 공통 유틸리티 함수 모음
│   ├── vision_module.py            # YOLOv8 비전 분석 및 감지 모듈
│   └── yolov8n.pt                  # YOLO 기본 가중치 파일
├── frontend/                       # React / Vite 프론트엔드 애플리케이션
│   ├── public/                     # 정적 퍼블릭 에셋 파일
│   ├── src/                        # 프론트엔드 소스코드
│   │   ├── assets/                 # 이미지 및 스타일 자원
│   │   ├── components/             # UI 컴포넌트 폴더
│   │   │   ├── common/             # 공통 컴포넌트 (ImageModal.jsx)
│   │   │   └── dashboard/          # 대시보드 탭별 컴포넌트 모음
│   │   ├── hooks/                  # 커스텀 훅 폴더
│   │   ├── pages/                  # 페이지 단위 컴포넌트 (DashboardPage.jsx, LoginPage.jsx)
│   │   ├── services/               # API 통신 서비스 모듈
│   │   ├── App.css                 # 애플리케이션 공통 스타일
│   │   ├── App.jsx                 # 루트 컴포넌트
│   │   ├── index.css               # 글로벌 CSS 설정
│   │   └── main.jsx                # React 앱 진입점
│   ├── .gitignore                  # 프론트엔드 예외 처리 설정
│   ├── eslint.config.js            # ESLint 코드 린트 설정
│   ├── index.html                  # HTML 문서 템플릿
│   ├── package.json                # 노드 패키지 정보 및 의존성
│   ├── package-lock.json           # 패키지 버전 고정 파일
│   ├── README.md                   # 프론트엔드 설명서
│   └── vite.config.js              # Vite 번들러 설정 파일
├── manual/                         # 소모품 정비 매뉴얼 데이터
│   └── volvo/
│       └── xc60/                   # 볼보 XC60 정비 가이드 데이터
│           ├── markdown/           # 마크다운 문서 원본
│           │   ├── volvo_xc60_air_cleaner.md
│           │   ├── volvo_xc60_cabin_filter.md
│           │   └── volvo_xc60_front_wiper.md
│           ├── volvo_xc60_air_cleaner.json
│           ├── volvo_xc60_cabin_filter.json
│           └── volvo_xc60_front_wiper.json
├── manual_images/                  # 부품 교체 단계별 상세 이미지 에셋
│   ├── air_cleaner/                # 에어 클리너 관련 이미지
│   ├── cabin_filter/               # 캐빈 필터 관련 이미지
│   └── front_wiper/                # 앞유리 와이퍼 관련 이미지
├── runs/                           # YOLO 모델 학습 결과물 및 로그 (Git 제외)
├── vector_db/                      # ChromaDB 벡터 스토어 데이터 저장소 (Git 제외)
├── yolo_training_data/             # YOLO 학습용 데이터셋 및 설정 (Git 제외)
├── .dockerignore                   # Docker 빌드 예외 처리 설정
├── .gitignore                      # 루트 레벨 Git 예외 처리 설정
├── Dockerfile                      # 도커 이미지 빌드 파일
├── docker-compose.yml              # 도커 멀티 컨테이너 오케스트레이션 설정
└── run.bat                         # 프로젝트 통합 실행 배치 스크립트
```

8. 트러블슈팅 및 주요 엔지니어링 의사결정 (Troubleshooting)
- 비전 인식 모델 신뢰도 최적화 (Threshold Tuning)
  - 문제 상황: 초기 학습 데이터 양이 부족하여, 실제 부품(예: `brake_disc`)임에도 기존의 엄격한 기준(0.75) 때문에 부품을 제대로 인식하지 못하고 실패 처리되는 현상 발생
  - 해결 및 결정: 모델의 유연성을 확보하고 사용자 경험을 개선하기 위해 Confidence Threshold를 0.60으로 조정
  - 상세 조치: 비전 모듈(`vision_module.py`)과 백엔드 API 검증단(`main.py`)의 기준치를 일치시켜 파이프라인 전체가 유기적으로 동작하도록 동기화
  - 인사이트: 데이터셋 확장 전 프로토타입 단계에서 발생할 수 있는 탐지율 저하 문제를 엔지니어링 관점에서 유연하게 대처

- 예외 처리 및 확장 가능한 아키텍처 (Graceful Degradation)
  - 현재 상태: `air_cleaner`, `front_wiper`, `cabin_filter`는 이미지 인식 및 상세 작업 가이드(메뉴얼)까지 완벽하게 구축되어 있으며, `spark_plug`, `brake_disc`, `brake_pad`, `oil_filter` 4개 품목은 현재 부품 인식(이미지 학습) 단계까지 추가로 확장 적용된 상태임
  - 설계 특징: 정비 매뉴얼 데이터가 아직 구축되지 않은 품목이 인식되거나 질의될 경우, 시스템이 에러로 중단되는 대신 방어 로직을 수행하도록 설계
  - 컨텍스트 관리: 인식 실패 또는 예외 상황 시 초기화 로직을 통해 이전 대화의 잔여물이 다음 스텝으로 넘어가는 현상을 방지하여 프로토타입 안정성 확보

- RAG 오조회(Misretrieval) 및 로그 연동 구조 파악
  - 현상: `spark_plug`, `brake_disc`, `brake_pad`, `oil_filter` 등 아직 전용 정비 매뉴얼이 등록되지 않은 품목 질의 시, 코사인 유사도 연산 특성으로 인해 엉뚱한 이전 가이드가 오조회되거나 질의 파싱 로직에 의해 `[일반 문의 - 교체 방법]`으로 통합 기록되는 현상 분석 완료
  - 대응: 향후 최소 유사도 점수(Threshold) 필터링 도입을 위한 아키텍처 개선 포인트를 트러블슈팅 문서화

- 원클릭 실행 스크립트(`run.bat`) 최적화
  - 개선 사항: 도커 컨테이너 내림(`down`) 시 `-v` 옵션을 제거하여 Ollama AI 모델 볼륨(`diy_ollama_data`)이 삭제되지 않도록 보존 처리 (재실행 시 4GB 대용량 모델 중복 다운로드 방지). 아울러 대화형 CLI 명령어로 인한 멈춤 현상을 방지하고자 무인 백그라운드 검증 방식(`ollama pull`)으로 스크립트 안정화 구축

9. 사전 준비 및 설치·실행 가이드

사전 준비 사항
- Docker Desktop (필수): 로컬 PC 환경에 도커가 설치되어 실행 중이어야 합니다.
- Node.js (v18+ 권장): 프론트엔드 구동용

원클릭 통합 자동 실행 (권장)

- 운영체제(Windows/macOS)에 맞춘 통합 스크립트로 기존 컨테이너 정리, 벡터 DB 자동 구축, 백엔드/AI 서버 구동, Ollama 모델 무인 검증, 프론트엔드 패키지 설치 및 서버 실행까지 전 과정을 원클릭으로 처리할 수 있습니다.

## Windows 환경

1. 프로젝트 다운로드 경로로 이동합니다.

2. run.bat 파일을 더블클릭하여 실행합니다.

## macOS / Linux 환경

1. 터미널을 열고 프로젝트 다운로드 폴더로 이동합니다.

2. 스크립트 실행 권한을 부여합니다. (최초 1회 필요)
- chmod +x run.sh
- 검증: ls -l run.sh 입력 시 파일 권한 항목에 x(실행 권한)가 포함되었는지 확인합니다.

3. run.sh 스크립트를 실행합니다.
- ./run.sh

## 웹 브라우저에서 아래 주소로 접속합니다.
- 프론트엔드 웹 UI: http://localhost:5173
- 백엔드 API 문서 (Swagger): http://localhost:8000/docs

수동 실행 방법 (Developer Manual)

1. 백엔드 도커 컨테이너 빌드 및 구동 (캐시 없이 새로 빌드 시: docker compose build --no-cache)
- docker compose up --build -d

2. 도커 컨테이너 내부에서 벡터 DB(ChromaDB) 초기 구축
## windows
- docker exec car-rag-app python backend/ingest.py

## macOS / Linux
- docker exec car-rag-app python3 backend/ingest.py

3. 백엔드 서버 재시작 (생성된 벡터 DB 완벽 반영)
- docker restart car-rag-app

4. LLaVA 비전 모델 확인 및 다운로드
- docker exec ollama-server ollama pull llava

5. 프론트엔드 구동
- cd frontend
- npm install
- npm run dev

6. 웹 브라우저 접속

- 접속 주소: http://localhost:5173/

[ 재실행 시 (이미 환경 세팅이 완료된 경우) ]

1. 루트 폴더에서 백엔드 백그라운드 구동: docker compose up -d

2. 프론트엔드 폴더에서 개발 서버 실행: cd frontend && npm run dev