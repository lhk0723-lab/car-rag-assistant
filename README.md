...
- 비전 인식 모델 신뢰도 최적화 (Threshold Tuning)
  - 문제 상황: 초기 학습 데이터 양이 부족하여, 실제 부품(예: `brake_disc`)임에도 기존의 엄격한 기준(0.75) 때문에 인식 실패(`False Negative`) 처리되는 현상 발생
...
- 예외 처리 및 확장 가능한 아키텍처 (Graceful Degradation)
  - 현재 상태: 주요 핵심 부품(`air_cleaner`, `front_wiper` 등) 위주로 작업 가이드(메뉴얼)가 우선 구축되어 있으며, 일부 부품은 데이터 보완 단계에 있음
  - 설계 특징: 매뉴얼 데이터가 없는 부품이 입력되거나 인식에 실패할 경우, 시스템이 에러를 발생시키며 중단되는 대신 방어 로직 수행
  - 컨텍스트 관리: 인식 실패 또는 예외 상황 시 `else` 블록에 초기화 로직(`setLastActivePart(null)`)을 구현하여 이전 대화의 잔여물이 다음 스텝으로 넘어가는 '컨텍스트 누수(Context Leakage)' 방지 및 프로토타입 안정성 확보
...
- Windows 환경에서는 루트 폴더에 포함된 run.bat 스크립트 하나로 클린 빌드(--no-cache), 컨테이너 구동, 벡터 DB 자동 구축, 프론트엔드 패키지 설치 및 서버 실행까지 전 과정을 원클릭으로 처리할 수 있습니다.

1. 프로젝트 루트 폴더 경로(C:\DIY)로 이동합니다.
