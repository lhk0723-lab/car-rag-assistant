        ...
        with st.spinner("🔍 분석 중입니다..."):
            ...
            if image_path and os.path.exists(image_path) and uploaded_file is not None:
                detected_class_name, display_image, confidence, err_msg = analyze_image_with_yolov8(
                    image_path, cabin_model, confidence_threshold=0.75
                )

                if detected_class_name:
                    # 감지된 클래스 이름(예: cabin_filter)을 직접 JSON 파일명과 정확히 매핑
                    target_json_path = os.path.join("data", "volvo", "xc60", f"{detected_class_name}.json")
                    ...
                    if is_asking_guide:
                        show_guide = True
                        response_text = f"🛠️ 사진 인식 결과 **[{part_display_name}]**(확률: {confidence:.1f}%)로 확인되어, 요청하신 볼보 XC60 상세 교체 가이드를 출력합니다."
                    else:
            ...
            # [시나리오 2] 사진 없이 텍스트로만 질문한 경우
                ...
                if matched_path and is_asking_guide:
                    ...
                    show_guide = True
                    response_text = "🛠️ 요청하신 볼보 XC60 정비 가이드 및 단계별 작업 순서입니다."
                elif matched_path:
                    target_json_path = matched_path
