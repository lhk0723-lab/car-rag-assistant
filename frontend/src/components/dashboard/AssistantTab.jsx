    ...
    <div className="flex-1 flex flex-col bg-[#0d1322]/70 border border-gray-800/80 rounded-2xl shadow-2xl backdrop-blur-sm overflow-hidden">
      <div className="px-6 py-4 border-b border-gray-800/80 bg-[#0d1322] flex items-center justify-between">
        <div className="flex items-center gap-3">
          ...
          <div>
            ...
            <p className="text-[11px] text-gray-400">사진 첨부 시 부품 이름 확인, 텍스트 입력 시 RAG 정비 가이드를 제공합니다.</p>
          </div>
        </div>
      </div>
      ...
      <div className="flex-1 p-6 overflow-y-auto space-y-6">
        {messages.map((msg, index) => {
          // 현재 매뉴얼 메시지 안에 있는 모든 스텝의 이미지를 하나의 통합 배열로 미리 추출
        ...
        {loading && <div className="text-gray-400 text-xs px-11 animate-pulse">AI가 이미지를 분석하고 있습니다...</div>}
      </div>

      <form onSubmit={(e) => handleSendMessage(e, null)} className="p-4 border-t border-gray-800/80 bg-[#0d1322]/50 flex flex-col gap-2.5">
        {previewUrl && (
          <div className="flex items-center justify-between bg-[#090d16] px-4 py-2.5 rounded-xl border border-emerald-500/40 shadow-md">
            ...
            <div className="flex items-center gap-2">
              <button 
                type="button" 
                onClick={(e) => handleSendMessage(e, 'diagnose')} 
        ...
        <div className="flex items-center gap-3">
          <label className="p-3 bg-[#090d16] hover:bg-gray-800 text-gray-400 hover:text-emerald-400 border border-gray-800 rounded-xl cursor-pointer transition flex items-center gap-1.5 text-xs font-medium" title="사진 첨부하기">
            ...
            <input type="file" accept="image/*" onChange={(e) => { const f = e.target.files[0]; if(f){ setSelectedImage(f); setPreviewUrl(URL.createObjectURL(f)); } }} className="hidden" />
          </label>
          <input type="text" value={inputMessage} onChange={(e) => setInputMessage(e.target.value)} placeholder={`${carModel} 정비 관련 내용을 질문해보세요 (예: 먼지필터 교체방법)...`} className="flex-1 bg-[#090d16] border border-gray-800 rounded-xl px-4 py-3 text-sm focus:outline-none focus:border-emerald-500 text-white" />
